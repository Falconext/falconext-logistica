        ...
        <div className="max-w-[1400px] mx-auto animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">{t('trabajadores.lista.titulo')}</h1>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead>
                            <tr className="border-b border-slate-100 text-slate-400">
                                ...
                                <th className="px-5 py-3.5 font-medium text-right">{t('trabajadores.lista.colAcciones')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
                                    ...
                                    <tr key={w.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                        ...
                                        <td className="px-5 py-3.5">
                                            <div className="flex items-center justify-end gap-1.5">
                                                <Link href={`/trabajadores/${w.id}`} title={t('trabajadores.lista.verTitulo')} className="w-8 h-8 rounded-lg border border-slate-200 hover:bg-slate-100 flex items-center justify-center text-slate-500 transition">
                                                    <Eye size={15} />
                                                </Link>
                                                <button onClick={() => openEdit(w)} title={t('trabajadores.lista.editarTitulo')} className="w-8 h-8 rounded-lg border border-slate-200 hover:bg-slate-100 flex items-center justify-center text-slate-500 transition">
                                                    <Pencil size={15} />
                                                </button>
                                                <button onClick={() => setToDelete(w)} title={t('trabajadores.lista.eliminarTitulo')} className="w-8 h-8 rounded-lg border border-slate-200 hover:bg-red-50 hover:border-red-200 hover:text-red-500 flex items-center justify-center text-slate-500 transition">
                                                    <Trash2 size={15} />
                                                </button>
                                            </div>
                                        </td>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm border border-slate-200 shadow-2xl p-6 max-h-[92vh] overflow-y-auto animate-in slide-in-from-bottom sm:zoom-in-95 duration-200">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-slate-900">{t('trabajadores.lista.eliminarModalTitulo')}</h3>
                                <p className="text-sm text-slate-500 mt-1">
                                    {(() => {
                                        const [pre, post] = t('trabajadores.lista.eliminarModalMensaje').split('{nombre}');
                                        return <>{pre}<span className="font-semibold text-slate-700">{toDelete.nombre_completo}</span>{post}</>;
                                    })()}
                                </p>
                            </div>
                        </div>
