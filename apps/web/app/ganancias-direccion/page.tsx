        ...
        <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
            <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-extrabold text-slate-800">Ganancias</h1>
                        <p className="text-sm text-slate-500">Cuánto va ganando cada chofer/supervisor</p>
                    </div>
                </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-center">
                <div className="text-sm text-slate-500">Total a pagar (mes)</div>
            ...
            {loading ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">
                    <p className="font-semibold text-slate-700">Sin actividad</p>
                    <p className="text-sm">Aún no hay recorridos ni reperibilità este mes.</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {data.choferes.map((f) => (
                        <div key={f.trabajadorId} className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2">
                            <div className="flex items-center justify-between gap-2">
                                <div className="min-w-0">
                                    <div className="font-bold text-slate-800 truncate">{f.nombre}</div>
                                    {f.cargo && <div className="text-xs text-slate-500">{f.cargo}</div>}
                                </div>
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-600">
                                ...
                                <span className="text-slate-500">Pago horas: {format(f.pagoHoras)}</span>
                            </div>
                        </div>
                    ))}
                </div>
