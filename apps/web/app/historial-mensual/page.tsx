    ...
    return (
        <div className="p-4 md:p-6 w-full space-y-4">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-xl font-extrabold text-slate-800 dark:text-slate-100">Historial</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Tus km y entregas por mes</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 p-10 text-center text-slate-500 dark:text-slate-400">
                    <p className="font-semibold text-slate-700 dark:text-slate-200">Sin historial</p>
                ...
                <div className="space-y-3">
                    {meses.map((m) => {
                        ...
                        return (
                            <div key={key} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
                                <button onClick={() => toggle(m)} className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800/50">
                                    <span className="font-extrabold text-slate-800 dark:text-slate-100 capitalize">{m.label}</span>
                                    {isOpen ? <ChevronUp size={18} className="text-slate-400" /> : <ChevronDown size={18} className="text-slate-400" />}
                                </button>
                                <div className="px-4 pb-3 flex flex-wrap gap-x-5 gap-y-1 text-sm text-slate-600 dark:text-slate-400">
                                ...
                                {isOpen && (
                                    <div className="border-t border-slate-100 dark:border-slate-800 px-4 py-3 space-y-2">
                                        <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">¿Por qué este total?</div>
                                        ...
                                        <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase pt-2">Recorridos ({m.recorridos ?? 0})</div>
                                        {det === 'loading' ? (
                                            ...
                                            <div className="divide-y divide-slate-100">
                                                {det.map((it, i) => (
                                                    <div key={i} className="py-2">
                                                        <div className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{it.cliente}</div>
